...
function getUserDisplayEle(user) {
  return `<div class="user-display-ele">
      ...
      <div class="normal-view">
        ...
        <button class="edit-user-btn" data-user-id="${user._id}" data-user-role="${user.role}">
          Edit
        </button>
...
function addUser() {
  ...
  // Call api
  Http.post("/api/user/register", data).then(() => displayFiles());
}
...
function submitEdit(ele) {
  ...
  Http.put("/api/user/update", data).then(() => displayFiles());
}
...
function deleteUser(ele) {
  var id = ele.getAttribute("data-user-id");
  Http.delete("/api/user/delete/" + id).then(() => displayFiles());
}
